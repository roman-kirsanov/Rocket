...
void StringInsert(std::string& string, std::int64_t position, std::string const& substring) {
    ...
    static thread_local auto _codepoints1 = std::vector<std::uint32_t>();
    static thread_local auto _codepoints2 = std::vector<std::uint32_t>();

    if (position <= 0) {
        string.insert(0, substring);
    } else if (position >= string.size()) {
        string.append(substring);
    } else {
        StringToCodepoints(string, _codepoints1);
        StringToCodepoints(substring, _codepoints2);

        _codepoints1.insert(_codepoints1.begin() + position, _codepoints2.begin(), _codepoints2.end());

        StringFromCodepoints(_codepoints1, string);
    }
}
...
std::string StringReplace(std::string const& string, std::string const& oldSubstring, std::string const& newSubstring) {
    ...
    auto ret = string;
    auto len = oldSubstring.length();
    auto it = ret.find(oldSubstring);
    if (it != std::string::npos) {
        ret.replace(it, len, newSubstring);
    }
    return ret;
}
...
std::uint32_t CodepointToUpper(std::uint32_t codepoint) {
    ...
    for (std::int64_t i = 0; i < 1250; i++) {
        if (_lowerupper[i][0] == codepoint) {
            return _lowerupper[i][1];
        }
    }
...
std::uint32_t CodepointToLower(std::uint32_t codepoint) {
    ...
    for (std::int64_t i = 0; i < 1250; i++) {
        if (_lowerupper[i][1] == codepoint) {
            return _lowerupper[i][0];
        }
    }
...
bool CodepointIsWordChar(std::uint32_t codepoint) {
    ...
    /* ponytail: pragmatic approximation — full Unicode word classes if ever needed. */
    if (codepoint >= 0x80) {
        return !CodepointIsWhitespace(codepoint);
    }
...
void StringToCodepoints(std::string const& string, std::vector<std::uint32_t>& codepoints) {
    ...
    auto data = reinterpret_cast<std::uint8_t const*>(string.data());
    ...
    for (;;) {
        ...
        if (size == 0) {
            ...
            /* A stray continuation byte at a sequence start decodes to the replacement character. */
...
void StringFromCodepoints(std::vector<std::uint32_t> const& codepoints, std::string& string) {
    ...
    for (auto& codepoint : codepoints) {
        ...
        if (codepoint <= 0x7F) {
            ...
            out[0] = ((codepoint >> 6) & 037) | 0300;
            out[1] = ((codepoint & 077) | 0200);
            length = 2;
        } else if (codepoint <= 0xFFFF) {
            out[0] = ((codepoint >> 12) & 017) | 0340;
            out[1] = ((codepoint >> 6) & 077) | 0200;
            out[2] = ((codepoint & 077) | 0200);
            length = 3;
        } else if (codepoint <= 0x10FFFF) {
            out[0] = ((codepoint >> 18) & 070) | 0360;
            out[1] = ((codepoint >> 12) & 077) | 0200;
            out[2] = ((codepoint >> 6) & 077) | 0200;
            out[3] = ((codepoint & 077) | 0200);
            length = 4;
        }
...
std::string StringToLower(std::string const& string) {
    ...
    auto copy = string;
    std::transform(string.begin(), string.end(), copy.begin(), [](auto c) {
...
std::int64_t GraphemeNext(std::vector<std::uint32_t> const& codepoints, std::int64_t index) {
    ...
    auto size = (std::int64_t)codepoints.size();
    ...
    for (;;) {
        auto base = codepoints[index];
        index += 1;

        if (base == '\r') {
            if (index < size && codepoints[index] == '\n') {
                index += 1;
            }
            break;
        }
        ...
        /* Hangul syllable sequences (UAX #29 GB6-GB8) */
        while (index < size) {
            auto const next = codepoints[index];
            auto const joins = (
                (_GraphemeIsHangulL(base) && (_GraphemeIsHangulL(next) || _GraphemeIsHangulV(next) || _GraphemeIsHangulLV(next) || _GraphemeIsHangulLVT(next))) ||
                ((_GraphemeIsHangulLV(base) || _GraphemeIsHangulV(base)) && (_GraphemeIsHangulV(next) || _GraphemeIsHangulT(next))) ||
                ((_GraphemeIsHangulLVT(base) || _GraphemeIsHangulT(base)) && _GraphemeIsHangulT(next))
            );
            if (joins == false) {
                break;
            }
        ...
        while (index < size && _GraphemeIsExtender(codepoints[index])) {
            index += 1;
        }

        if (index < size && codepoints[index] == 0x200D && index + 1 < size) {
            index += 1;
            continue;
        }
...
std::int64_t GraphemePrev(std::vector<std::uint32_t> const& codepoints, std::int64_t index) {
    ...
    while (position < index) {
        boundary = position;
        position = GraphemeNext(codepoints, position);
    }
